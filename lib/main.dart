...
const SEARCH_ENGINE_ID_GOOGLE = "*"; // self

const SEARCH_ENGINE_ID_TWITTER = "*";
const SEARCH_ENGINE_ID_FACEBOOK = "*";
const SEARCH_ENGINE_ID_INSTAGRAM = "*";
const SEARCH_ENGINE_ID_LINKEDIN = "*";
    ...
    switch (platform) {
      ...
      case 'Bing':
        response = !_gg
            ? await http.get(
                Uri.parse(
                    'https://api.bing.microsoft.com/v7.0/custom/search?q=$value&customconfig='),
...
_imageSearchGoogle(src, path, [imgURL = "", type]) async {
  ...
  const myJsonAsString =
      '{ "type": "service_account", "project_id": "*", "private_key_id": "*", "private_key": "*"}';
  ...
  try {
    ...
    Future textDetection(String image, Map<String, dynamic> GoogleObj) async {
      ...
      if (entities == null) {
        ...
        List ocr = [];
        final _WebViewContainerState keywords = _WebViewContainerState();

        for (int i = 0; i < entities!.length; i++) {
          ocr.add((entities![i].description?.trim()));
        }

        var extracted = await keywords._extractKeywords(ocr.toString());
        //extracted = <dynamic>{...keywords._searchText.split(' '), ...extracted}
        //.toList();
    ...
    Future<Map> webSearch(String image, Map<String, dynamic> GoogleObj) async {
      ...
      _response.responses?.forEach((data) {
        ...
        if (data.webDetection?.fullMatchingImages != null) {
          fullMatchImage =
              data.webDetection!.fullMatchingImages as List<vision.WebImage>;
          log("not null | ${data.webDetection?.fullMatchingImages}");
        } else {
          log("null1");
        }
      ...
      if (lens == 0) {
        ...
        for (j = 0; j < lens; j++) {
          urls.add({
            ...
            'source': pageWithMatchImage![j].url.toString()
          });
          //}
        }
      }
...
_imageSearch(src, path, [imgURI = ""]) async {
  ...
  String bestguessBing = "";
  if (bing["bestGuessList"] != null) {
    bestguessBing = bing["bestGuessLabel"][0];
  }
